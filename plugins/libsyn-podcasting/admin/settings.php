...
if (isset($_REQUEST['submit']) || isset($_REQUEST['libsyn_settings_submit'])) { //has showSelect on form.
	...
	if($api instanceof \Libsyn\Api) { //Brand new setup or changes?
		if(!empty($_REQUEST['submit']) && (($_REQUEST['submit']==='Save Player Settings' || $_REQUEST['submit']==='Save+Player+Settings') || ($_REQUEST['submit']==='Save Settings' || $_REQUEST['submit']==='Save+Settings'))) { //has Player Settings Update
			...
			if(!empty($_REQUEST['clear-settings-data'])) {
				...
				if($check === true) {
					...
					if($remove_settings) {
						...
						$msg = "Something went wrong when trying to clear settings";
						$error_message = "&error=true";
					}
					echo $plugin->redirectUrlScript($plugin->admin_url('admin.php').'?page='.LIBSYN_DIR.'/admin/settings.php&msg='.$msg.$error_message); 
				} else {
					$msg = "There was a problem when trying to clear settings";
			...
			if(!isset($_REQUEST['settings_add_podcast_metadata'])) $additionalSettings['settings_add_podcast_metadata'] = '';
				else $additionalSettings['settings_add_podcast_metadata'] = $_REQUEST['settings_add_podcast_metadata'];
	...
	if(!$authorized) {
		if(isset($libsyn_code)) { //handle auth callback $_REQUEST['libsyn_code']
			...
			if(!empty($html)) {
				...
				$html = '
					<h3 style="color:red;">Something went wrong with your authentication process.</h3>
					<p>Your client information may not be valid or something is wrong with the Libsyn authentication servers.</p>
					<p>It is recommended that you clear your settings and enter new client id and secret.</p>
					<div style="height: 124px;">
						<div style="float: left; width: 60%;">
							<form name="' . LIBSYN_NS . 'form' . '" id="' . LIBSYN_NS . 'form' . '" method="post" action="">
							<div>
								Clear Settings button below to continue.
								<br />
								<input type="hidden" name="clear-settings-data" value="' . time() . '" />
								<input type="submit" value="Clear Settings" class="button button-primary" id="clear-settings-button" name="submit" />
							</div>
							</form>
					</div>
				';
		  ...
		  <div id="post-body">
			...
			<?php if(!isset($setup_new)) { ?>
			  ...
			  <div class="box_left_column" id="libsyn_tabbed_settings_box">
				<div class="stuffbox" id="libsyn_player_settings">
				  <div class="inside box_left_content" style="display:none;">
					<h3>
						<label><a href="#libsyn_player_settings" style="color:black;text-decoration:none;" class="libsyn_player_settings_anchor libsyn-text-underline">Player Settings</a>&nbsp;&nbsp;&nbsp;<a href="#libsyn_additional_settings" style="color:darkgray;text-decoration:none;" class="libsyn_additional_settings_anchor">Additional Settings</a></label>
					</h3>
					<div class="inside">
				...
				<div class="stuffbox" id="libsyn_additional_settings" style="display:none;">
				  <div class="inside box_left_content" style="display:none;">
					<h3>
						<label><a href="#libsyn_player_settings" style="color:darkgray;text-decoration:none;" class="libsyn_player_settings_anchor">Player Settings</a>&nbsp;&nbsp;&nbsp;<a href="#libsyn_additional_settings" style="color:black;text-decoration:none;" class="libsyn_additional_settings_anchor libsyn-text-underline">Additional Settings</a></label>
					</h3>
					<div class="inside">
						<p id="player-description-text"><em>Extra settings that are optional in most podcast configurations.</em></p>
						<div class="box_clear"></div>
						<table class="form-table">
							<tr valign="top">
								<th colspan="2"><input style="margin-left: 2px;" id="settings_add_podcast_metadata" type="checkbox" value="add_podcast_metadata" name="settings_add_podcast_metadata" />&nbsp;Add Podcast Feed Headers?&nbsp;&nbsp;<em>(This will add your Podcast Metadata to Wordpress Pages/Posts)</em></th>
								<td>
								</td>
							</tr>
							<tr valign="bottom">
								...
								<td>
									<br />
										<input type="submit" value="Save Settings" class="button button-primary" id="additional-settings-submit" name="submit">
								</td>
							</tr>
							<tr valign="bottom">
			  ...
			  <div class="box_right_column">
				  ...
				  <div class="stuffbox">
					<div class="inside box_right_content_3" style="display:none">
						<h3>
							<label>Clear Settings / Debug</label>
						</h3>
						<div class="inside">	
							<div class="box_clear"></div>
			  ...
			  <div id="clear-settings-dialog" class="hidden" title="Confirm Clear Settings">
				<p><span style="color:red;font-weight:600;">Warning!</span> By accepting you will be removing all your Libsyn Publisher Hub settings.  Click yes to continue.</p>
				<br>
			  </div>
		...
		(function($){
			$(document).ready(function(){
				...
				$(".box_left_content").fadeIn('fast', function() {
					...
					var settingsAddPodcastMetadata = '<?php echo $settingsAddPodcastMetadata; ?>';
					if(settingsAddPodcastMetadata == 'add_podcast_metadata') {
						$('#settings_add_podcast_metadata').attr('checked', true);
					}
				});
			});
		})(jQuery);
